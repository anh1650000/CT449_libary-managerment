...
const login = async (req, res) => {
  try {
    ...
    if (account.refModel === 'User') {
      ...
      if (user) userInfo = { userId: account.refId, maDocGia: user.maDocGia, fullName: `${user.hoLot} ${user.ten}`, email: account.email, role: account.role }
    } else if (account.refModel === 'Staff') {
      const staff = await Staff.findById(account.refId);
      if (staff) userInfo = { userId: account.refId, msnv: staff.msnv, hoTenNV: staff.hoTenNV, chucVu: staff.chucVu, role: account.role };
    }
